        ...
        internal static string Start(List<string> Tokens)
        {
            ...
            for (int i = 0; i < Tokens.Count; i++)
            {
                ...
                if(i == Tokens.Count - 1)
                {
                    ...
                    if (Matchvar.Success) { Variables.Add(Matchvar.Groups[1].Value.Trim(), Lexer.Start(Matchvar.Groups[2].Value.Trim())); continue; }
                    ...
                    if (MatchAritmetica.Success) { return Calculo.Evaluar(value); }
            ...
            foreach (string reservada in Palabras_Reservadas.Palabras)
            {
                ...
                return "!SYNTAX ERROR: Invalid token " + Tokens[0] + " => " + value;
